...
#' 
#' @description A sphere of radius R is drawn around each point in the trajectory. The number of
#' revisits is calculated as the number of segments of the trajectory passing through that sphere. 
#'  
#' @details For each point in the trajectory, a sphere of radius R is drawn around that point.
...
#' the trajectory data in five columns (the x-coordinate, the y-coordinate, the z-coordinate, the datetime, and the animal id). 
#' @param radius numeric radius to use in units of the (x,y,z) location data to detect recursions.
...
#' @seealso \code{\link{getRecursions3DAtLocations}}
#' 
#' @examples
#' data(martin)
...
#' 
#' @export
#' @name getRecursions3D
#' 
getRecursions3D <- function(x, radius, threshold = 0, timeunits = c("hours", "secs", "mins", "days"), verbose = TRUE)
{
	UseMethod('getRecursions3D', x)
}
